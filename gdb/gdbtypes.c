...
alloc_type (struct objfile *objfile)
{
  ...
  type = OBSTACK_ZALLOC (&OBJFILE_OBSTACK (objfile), struct type);
  TYPE_MAIN_TYPE (type) = OBSTACK_ZALLOC (&OBJFILE_OBSTACK (objfile),
					  struct main_type);
...
alloc_type_instance (struct type *oldtype)
{
  ...
  if (! TYPE_OBJFILE_OWNED (oldtype))
    ...
    type = OBSTACK_ZALLOC (&OBJFILE_OBSTACK (TYPE_OBJFILE (oldtype)),
			   struct type);
		     ...
		     htab_t copied_types)
{
  ...
  stored = obstack_alloc (&OBJFILE_OBSTACK (objfile), sizeof (struct type_pair));
  stored->old = type;
  stored->new = new_type;
  *slot = stored;
...
objfile_type (struct objfile *objfile)
{
  ...
  objfile_type = OBSTACK_CALLOC (&OBJFILE_OBSTACK (objfile),
