  ...
  for (lm = link_map_start (); lm; )
    {
      ...
      new_so_list = (struct so_list *) xmalloc (sizeof (struct so_list));
      old_chain = make_cleanup (xfree, new_so_list);

      memset (new_so_list, 0, sizeof (*new_so_list));
      new_so_list->lm_info
	...
	{
	  strncpy (new_so_list->so_name, namebuf, SO_NAME_MAX_PATH_SIZE - 1);
	  new_so_list->so_name[SO_NAME_MAX_PATH_SIZE - 1] = '\0';
	  xfree (namebuf);
	  strcpy (new_so_list->so_original_name, new_so_list->so_name);
	}

	if (new_so_list->so_name[0] && !match_main (new_so_list->so_name))
	  {
	    struct lm_info *lmi = new_so_list->lm_info;
	    unsigned int tmp;
...
#define EXTRACT(_fld) \
  extract_unsigned_integer ((gdb_byte *)&dbuf._fld, \
	    ...
	    printf ("\n+ library \"%s\" is described at %s\n",
		    new_so_list->so_name, paddress (target_gdbarch, lm));
	    printf ("  'version' is %d\n",
		    new_so_list->lm_info->struct_version);
	    printf ("  'bind_mode' is %d\n", new_so_list->lm_info->bind_mode);
	    printf ("  'library_version' is %d\n", 
		    new_so_list->lm_info->library_version);
	    printf ("  'text_addr' is %s\n",
		    paddress (target_gdbarch, new_so_list->lm_info->text_addr));
	    printf ("  'text_link_addr' is %s\n",
		    paddress (target_gdbarch,
			      new_so_list->lm_info->text_link_addr));
	    printf ("  'text_end' is %s\n",
		    paddress (target_gdbarch, new_so_list->lm_info->text_end));
	    printf ("  'data_start' is %s\n",
		    paddress (target_gdbarch,
			      new_so_list->lm_info->data_start));
	    printf ("  'bss_start' is %s\n",
		    paddress (target_gdbarch, new_so_list->lm_info->bss_start));
	    printf ("  'data_end' is %s\n",
		    paddress (target_gdbarch, new_so_list->lm_info->data_end));
	    printf ("  'got_value' is %s\n",
		    paddress (target_gdbarch, new_so_list->lm_info->got_value));
	    printf ("  'tsd_start_addr' is %s\n",
		    paddress (target_gdbarch,
			      new_so_list->lm_info->tsd_start_addr));
#endif

	    new_so_list->addr_low = lmi->text_addr;
	    new_so_list->addr_high = lmi->text_end;

	    /* Link the new object onto the list.  */
	    new_so_list->next = NULL;
