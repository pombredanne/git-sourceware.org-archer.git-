      ...
      if (task == task_self ())
	{
	  if (port == task_self())
	      ...
	      else
		{
		  ...
		  for (k = 0; k < inf->private_data->exception_info.count; k++)
		    if (port == inf->private_data->exception_info.ports[k])
		      {
		  ...
		  if (inf->private_data->threads)
		    {
		      for (k = 0;
			   VEC_iterate(darwin_thread_t,
				       inf->private_data->threads, k, t);
			   k++)
			if (port == t->gdb_port)
			  {
			    printf_unfiltered (_(" inferior-thread for 0x%x"),
					       inf->private_data->task);
			    break;
			  }
		    }
		}
...
info_mach_region_command (char *exp, int from_tty)
{
  ...
  darwin_debug_region (inf->private_data->task, address);
}
...
info_mach_exceptions_command (char *args, int from_tty)
{
  ...
  if (args != NULL)
      ...
      if (strcmp (args, "saved") == 0)
	{
	  if (ptid_equal (inferior_ptid, null_ptid))
	    printf_unfiltered (_("No inferior running\n"));
	  disp_exception (&current_inferior ()->private_data->exception_info);
	  return;
	}
	...
	(inf->private_data->task, EXC_MASK_ALL, info.masks,
	 &info.count, info.ports, info.behaviors, info.flavors);
      MACH_CHECK_ERROR (kret);
      disp_exception (&info);
    }
