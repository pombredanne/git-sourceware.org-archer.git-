	       ...
	       struct objfile *objfile)
{
  ...
  switch (deftype)
	...
	case 's':
	  {
	    ...
	    SYMBOL_TYPE (sym) = create_array_type (NULL,
				  ...
				  range_type);
	    string_value = obstack_alloc (&OBJFILE_OBSTACK (objfile), ind + 1);
      ...
      if (synonym)
        {
          ...
          *struct_sym = *sym;
          ...
          if (TYPE_NAME (SYMBOL_TYPE (sym)) == 0)
            TYPE_NAME (SYMBOL_TYPE (sym)) = obconcat (&OBJFILE_OBSTACK (objfile),
						      SYMBOL_LINKAGE_NAME (sym),
						      (char *) NULL);
	...
	TYPE_TAG_NAME (SYMBOL_TYPE (sym)) = obconcat (&OBJFILE_OBSTACK (objfile),
						      SYMBOL_LINKAGE_NAME (sym),
						      (char *) NULL);
      add_symbol_to_list (sym, &file_symbols);
  ...
  switch (type_descriptor)
...

	{
	  ...
	  if (type_name == NULL)
	    {
	      to = type_name =
		(char *) obstack_alloc (&OBJFILE_OBSTACK (objfile), p - *pp + 1);
	...
	for (ppt = file_symbols; ppt; ppt = ppt->next)
	  for (i = 0; i < ppt->nsyms; i++)
		...
		{
		  obstack_free (&OBJFILE_OBSTACK (objfile), type_name);
  ...
  if (!negative_types)
    {
      ...
      negative_types = OBSTACK_CALLOC (&OBJFILE_OBSTACK (objfile),
      ...
      else
	{
	  ...
	  */
	  ...
	  if (has_destructor && has_other)
		...
		= obconcat (&OBJFILE_OBSTACK (objfile), "~",
		 ...
		 struct objfile *objfile)
{
  ...
  if (*++p == 'v')
	...
	case 'f':		/* $vf -- a virtual function table pointer */
	  ...
	  fip->list->field.name = obconcat (&OBJFILE_OBSTACK (objfile),
	...
	case 'b':		/* $vb -- a virtual bsomethingorother */
	  ...
	  fip->list->field.name = obconcat (&OBJFILE_OBSTACK (objfile), vb_name,
					    name, (char *) NULL);
	  break;
		       ...
		       struct type *type, struct objfile *objfile)
{
  ...
  fip->list->field.name =
    obsavestring (*pp, p - *pp, &OBJFILE_OBSTACK (objfile));
  *pp = p + 1;
  ...
  while (**pp && **pp != ';' && **pp != ',')
    {
      ...
      while (*p != ':')
	p++;
      name = obsavestring (*pp, p - *pp, &OBJFILE_OBSTACK (objfile));
      *pp = p + 1;
