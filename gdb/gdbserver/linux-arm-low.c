...
update_registers_callback (struct inferior_list_entry *entry, void *arg)
{
  ...
  if (pid_of (thread) == pid_of (cs->ss->current_thread))
    {
		  ...
		  int len, struct raw_breakpoint *bp)
{
  ...
  if (watch)
    {
      ...
      pts = proc->piprivate->arch_private->wpts;
    }
  else
    {
      count = arm_linux_get_hw_breakpoint_count ();
      pts = proc->piprivate->arch_private->bpts;
    }
  ...
  for (i = 0; i < count; i++)
	...
	pts[i] = p;
	find_inferior (&cs->ss->all_threads, update_registers_callback, &data);
	return 0;
      }
...
arm_prepare_to_resume (struct lwp_info *lwp)
{
  ...
  struct arch_process_info *proc_info = proc->piprivate->arch_private;
