...
linux_create_inferior (char *program, char **allargs)
{
  ...
  struct cleanup *restore_personality
    = maybe_disable_address_space_randomization (cs->disable_randomization);
		   ...
		   void *data)
{
  ...
  entry = find_inferior (&cs->ss->all_threads, iterate_over_lwps_filter, &args);
  if (entry == NULL)
    return NULL;
  ...
  while (event_child == NULL)
      ...
      if (ret > 0)
	{
	 ...
	 LWPs that don't have an interesting event to report.  */
      if (stopping_threads == NOT_STOPPING_THREADS)
	for_each_inferior (&cs->ss->all_threads, resume_stopped_resumed_lwps);
...
linux_handle_new_gdb_connection (void)
{
  ...
  find_inferior (&cs->ss->all_threads, reset_lwp_ptrace_options_callback , &pid);
}
